...
export default function AboutPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/content/about`)
      .then((res) => res.json())
      .then((data) => setContent(data))
      .catch(() => setContent(null));
  }, []);

  const mission = content ? (locale === 'ka' ? content.missionKa : content.missionEn) : null;
  const story = content ? (locale === 'ka' ? content.storyKa : content.storyEn) : null;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-900">
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">{t('storyTitle')}</h2>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {['simplicity', 'transparency', 'support'].map((value) => (
              <div key={value} className="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-lg">
      ...
      <section className="py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">{t('ctaTitle')}</h2>
          <p className="text-lg text-gray-600 dark:text-gray-300 mb-8">{t('ctaDescription')}</p>
          <Link
            href="/register"
...
function ValueIcon({ type }: { type: string }) {
  const iconClass = "w-6 h-6 text-[var(--accent-600)] dark:text-[var(--accent-400)]";
