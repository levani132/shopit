...
export default function ContactPage() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/content/contact`)
      .then((res) => res.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/content/contact/submit`, {
        method: 'POST',
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow-lg p-8">
              ...
              {submitStatus === 'success' ? (
                <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-6 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-green-800 dark:text-green-200 mb-2">{t('successTitle')}</h3>
            ...
            <div className="space-y-8">
              <div className="bg-gray-50 dark:bg-zinc-800 rounded-2xl p-8">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">{t('infoTitle')}</h2>
                <div className="space-y-6">
                  {contactInfo?.email && (
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h3 className="font-medium text-gray-900 dark:text-white">{t('emailLabel')}</h3>
                        <a href={`mailto:${contactInfo.email}`} className="text-[var(--accent-600)] hover:underline">
                          {contactInfo.email}
                        </a>
                      </div>
                    </div>
                  ...
                  {contactInfo?.phone && (
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h3 className="font-medium text-gray-900 dark:text-white">{t('phoneLabel')}</h3>
                        <a href={`tel:${contactInfo.phone}`} className="text-[var(--accent-600)] hover:underline">
                          {contactInfo.phone}
                        </a>
                      </div>
                    </div>
                  ...
                  {contactInfo?.address && (
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h3 className="font-medium text-gray-900 dark:text-white">{t('addressLabel')}</h3>
                  ...
                  {contactInfo?.workingHours && (
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h3 className="font-medium text-gray-900 dark:text-white">{t('hoursLabel')}</h3>
