        ...
        {viewMode === 'list' && (
          <div className="space-y-2">
            {upcomingStops.map((stop, idx) => {
              const isExpanded = expandedStopId === stop._id;
              ...
              return (
                <div
                  key={stop._id}
                  className={`bg-white dark:bg-zinc-800 rounded-lg border overflow-hidden ${
                ...
                >
                  {/* Clickable Header */}
                  <button
                    onClick={() => toggleStopExpansion(stop._id)}
                    className="w-full flex items-center gap-3 p-3 text-left hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition-colors"
                  >
                    <div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-zinc-700 flex items-center justify-center text-sm font-medium text-gray-600 dark:text-gray-400">
                      {activeRoute.currentStopIndex + idx + 2}
                    </div>
                  ...
                  {/* Expanded Content */}
                  {isExpanded && (
                    <div className="px-3 pb-3 pt-0 border-t border-gray-100 dark:border-zinc-700">
                      {/* Size badge */}
                      {sizeConfig && (
                        <div
                          className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs mt-2 ${sizeConfig.bg} ${sizeConfig.color}`}
                        >
  ...
  if (view === 'preview' && selectedRoute) {
    return (
      <div className="max-w-4xl mx-auto">
        ...
        {/* Stops List - Compact Accordion View */}
        {viewMode === 'list' && (
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {sizeConfig && (
                      <div
                        className={`hidden sm:flex items-center gap-1 px-2 py-0.5 rounded text-xs ${sizeConfig.bg} ${sizeConfig.color}`}
                      >
                        <span>{sizeConfig.icon}</span>
                      </div>
                    )}
                  ...
                  {isExpanded && (
                    <div className="p-3 border-t border-gray-100 dark:border-zinc-700">
                      {/* Size badge on mobile */}
                      {sizeConfig && (
                        <div
                          className={`sm:hidden inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs mb-3 ${sizeConfig.bg} ${sizeConfig.color}`}
                        >
                          <span>{sizeConfig.icon}</span>
                          <span className="font-medium">
                            {t(sizeConfig.label)}
