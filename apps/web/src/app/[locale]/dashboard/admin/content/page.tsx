...
export default function ContentManagementPage() {
  ...
  // About state
  const [about, setAbout] = useState<AboutContent>({ missionKa: '', missionEn: '', storyKa: '', storyEn: '' });
  ...
  // Legal state
  const [terms, setTerms] = useState<LegalContent>({ contentKa: '', contentEn: '' });
  const [privacy, setPrivacy] = useState<LegalContent>({ contentKa: '', contentEn: '' });
  ...
  const handleSaveContent = async (type: 'about' | 'contact' | 'terms' | 'privacy', data: unknown) => {
    setIsSaving(true);
    try {
      ...
      if (res.ok) {
        showMessage('success', `${type.charAt(0).toUpperCase() + type.slice(1)} saved successfully`);
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-1">Manage website content, FAQs, and legal pages</p>
      </div>

      {/* Message */}
      {message && (
        <div className={`p-4 rounded-xl ${message.type === 'success' ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300'}`}>
          {message.text}
        </div>
      )}
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow-sm p-6">
        {isLoading ? (
          ...
          <>
            ...
            {activeTab === 'faq' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('faqManagement')}</h2>
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => setEditingFaq({ questionKa: '', questionEn: '', answerKa: '', answerEn: '', category: 'general', order: 0, isActive: true })}
                ...
                {editingFaq && (
                  <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white dark:bg-zinc-800 rounded-2xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                      <h3 className="text-lg font-semibold mb-4">{editingFaq._id ? t('editFaq') : t('addFaq')}</h3>
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="block text-sm font-medium mb-1">Question (Georgian)</label>
                            <input
                              type="text"
                              value={editingFaq.questionKa || ''}
                              onChange={(e) => setEditingFaq({ ...editingFaq, questionKa: e.target.value })}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="block text-sm font-medium mb-1">Answer (Georgian)</label>
                            <textarea
                              rows={4}
                              value={editingFaq.answerKa || ''}
                              onChange={(e) => setEditingFaq({ ...editingFaq, answerKa: e.target.value })}
                              className="w-full px-3 py-2 border rounded-lg dark:bg-zinc-700 dark:border-zinc-600"
                            />
                          </div>
                          <div>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div>
                            <label className="block text-sm font-medium mb-1">{t('faqCategory')}</label>
                            <select
                          ...
                          <div>
                            <label className="block text-sm font-medium mb-1">{t('faqOrder')}</label>
                            <input
                              type="number"
                ...
                {faqs.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {faqs.map(faq => (
                      <div key={faq._id} className="border dark:border-zinc-700 rounded-lg p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <p className="font-medium text-gray-900 dark:text-white">{faq.questionKa}</p>
                            <p className="text-sm text-gray-600 dark:text-gray-400">{faq.questionEn}</p>
                          </div>
                          <div className="flex gap-2">
                            <button
            ...
            {activeTab === 'about' && (
              <div className="space-y-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('aboutManagement')}</h2>
                <div className="grid grid-cols-2 gap-4">
                  <div>
            ...
            {activeTab === 'contact' && (
              <div className="space-y-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('contactManagement')}</h2>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1">{t('socialFacebook')}</label>
                    <input
                      type="url"
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1">{t('socialInstagram')}</label>
                    <input
                      type="url"
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1">{t('socialLinkedIn')}</label>
                    <input
                      type="url"
            ...
            {activeTab === 'terms' && (
              <div className="space-y-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('termsManagement')}</h2>
                <p className="text-sm text-gray-500">You can use placeholders: {'{commissionPercent}'}, {'{courierPercent}'}, {'{minWithdrawal}'}</p>
                <div className="grid grid-cols-2 gap-4">
                  <div>
            ...
            {activeTab === 'privacy' && (
              <div className="space-y-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('privacyManagement')}</h2>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1">{t('contentKa')}</label>
                    <textarea
